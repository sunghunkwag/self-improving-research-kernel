"""Machine-check the bootstrapped RSI cold/warm proof."""
...
CONFIRMED_MARKER = "RECURSIVE SELF-IMPROVEMENT CONFIRMED"


def run_proof(*, budget: int, seeds: int, cold_seeds: int) -> dict:
    """Run the bootstrap proof and return a compact machine-readable result."""

    buffer = io.StringIO()
    with redirect_stdout(buffer):
        rsi_bootstrap.run_bootstrap(
            budget=budget,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    result = run_proof(
        budget=args.budget,
        seeds=args.seeds,
        cold_seeds=args.cold_seeds,
    )
    if args.json_output:
        args.json_output.parent.mkdir(parents=True, exist_ok=True)
        args.json_output.write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
    print(result["output"], end="")
    print(json.dumps({key: result[key] for key in ("budget", "seeds", "cold_seeds", "confirmed", "partial")}, sort_keys=True))
